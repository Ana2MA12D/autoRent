...
@section('content')
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="">
                <h1 style="text-align: center">Редактировать заказ аренды</h1>
                <form class="rental-order__create" action="{{ route('rental_orders.update', $rentalOrder->id) }}"
                      method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="client_id" name="client_id"
                            ...
                            @foreach($clients as $client)
                                <option
                                    value="{{ $client->id }}" {{ $rentalOrder->client_id == $client->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="car_id" name="car_id" aria-describedby="carHelp" required>
                            ...
                            @foreach($cars as $car)
                                <option value="{{ $car->id }}" {{ $rentalOrder->car_id == $car->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="pickup_date" name="pickup_date"
                               aria-describedby="pickupHelp" value="{{ $rentalOrder->pickup_date }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="dropoff_date" name="dropoff_date"
                               aria-describedby="dropoffHelp" value="{{ $rentalOrder->dropoff_date }}" required>
                        <div id="dropoffHelp" class="form-text">Укажите дату окончания аренды</div>
                    </div>

                    <div class="checkbox-item">
                        <input type="checkbox" id="confirm" name="confirm" required>
                        <label for="confirm">Подтверждаю редактирование заказа</label>
                    </div>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-between mt-3">
                        <button type="submit" class="create__btn">Обновить заказ</button>
